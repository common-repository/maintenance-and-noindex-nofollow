...
require_once(dirname(__FILE__).DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'lionscripts_plg_mnn.class.php');

$objLionTemp = new lionscripts_plg_mnn(basename(dirname(__FILE__)));
...
if(!is_admin())
{
	...
	{
		if(isset($LIONSCRIPTS[$objLionTemp->plg_identifier]['maintenance_mode']) && ($LIONSCRIPTS[$objLionTemp->plg_identifier]['maintenance_mode'] == 1))
		{
			echo '<center><br /><br />'.$LIONSCRIPTS[$objLionTemp->plg_identifier]['maintenance_text'].'</center><br /><br /><br /><br />'.$objLionTemp->attr_display(true);
			die();
		}
	}
}

if(!function_exists('lionscripts_mnn_uninstall'))
{
	function lionscripts_mnn_uninstall()
	{
